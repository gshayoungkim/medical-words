...
export async function upsertStudent({ id, nickname }) {
  const sql = getSql();
  const rows = await sql`
    INSERT INTO students (id, nickname, last_seen_at)
    VALUES (${id}, ${nickname}, NOW())
    ON CONFLICT (id) DO UPDATE
    SET nickname = EXCLUDED.nickname,
        last_seen_at = NOW()
    RETURNING id, nickname, created_at, last_seen_at
  `;
  return rows[0];
}
...
export async function getStudentStats() {
  ...
  const rows = await sql`
    SELECT
      ...
      COUNT(*) FILTER (WHERE last_seen_at >= NOW() - INTERVAL '7 days')::INTEGER AS active_week
    FROM students
  `;
  return rows[0] || { total: 0, active_week: 0 };
}
